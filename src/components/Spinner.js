...
/* STEP 1 */
  const [isSpinnneron, setIsSpinnerOn] = useState(true);

  const toggleSpinner = () => {
  /* STEP 4 */
  setIsSpinnerOn(!isSpinnneron)
  };
  ...
  return (
    <div className='widget-spinner container'>
      <h2>Spinner</h2>
      {
        isSpinnneron ? <div id='spinner' className='spinner'>--+--</div> : <div></div> /* STEP 2 */
      }
      <button id='toggleSpinner' onClick={toggleSpinner}>
         {isSpinnneron ? 'Hide Spinner' : "Show Spinner"}
      </button>
    </div>
  );
}
